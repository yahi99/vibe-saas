...
class SystemInistService {
    ...
    private void initConfigs() {
        // 1. 读取数据库全部配置信息
        Map<String, String> configs = yopsaasSystemConfigService.queryAll();
        ...
        for (Map.Entry<String, String> entry : DEFAULT_CONFIGS.entrySet()) {
            ...
            yopsaasSystemConfigService.addConfig(entry.getKey(), entry.getValue());
        }
    ...
    private Map<String, String> getSystemInfo() {
        ...
        infos.put("调试级别", environment.getProperty("logging.level.com.yongche.yopsaas.wx"));

        // 测试获取application-core.yml配置信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 1, "模块状态");
        infos.put("邮件", environment.getProperty("yopsaas.notify.mail.enable"));
        infos.put("短信", environment.getProperty("yopsaas.notify.sms.enable"));
        infos.put("模版消息", environment.getProperty("yopsaas.notify.wx.enable"));
        infos.put("快递信息", environment.getProperty("yopsaas.express.enable"));
        infos.put("快递鸟ID", environment.getProperty("yopsaas.express.appId"));
        infos.put("对象存储", environment.getProperty("yopsaas.storage.active"));
        infos.put("本地对象存储路径", environment.getProperty("yopsaas.storage.local.storagePath"));
        infos.put("本地对象访问地址", environment.getProperty("yopsaas.storage.local.address"));
        infos.put("本地对象访问端口", environment.getProperty("yopsaas.storage.local.port"));

        // 微信相关信息
        infos.put(SystemInfoPrinter.CREATE_PART_COPPER + 2, "微信相关");
        infos.put("微信APP KEY", environment.getProperty("yopsaas.wx.app-id"));
        infos.put("微信APP-SECRET", environment.getProperty("yopsaas.wx.app-secret"));
        infos.put("微信支付MCH-ID", environment.getProperty("yopsaas.wx.mch-id"));
